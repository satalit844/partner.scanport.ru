...
if (!function_exists('usertestSafeIdListTraining')) {
    function usertestSafeIdListTraining($value)
    {
        if (is_array($value)) {
            $items = $value;
        } else {
...
if (!function_exists('usertestSafeArrayTraining')) {
    function usertestSafeArrayTraining($value)
    {
        ...
        if($q = $modx->getObject('UserTestQuestions', $q_id['id'])){
            ...
            switch($q->type){
                ...
                case 2: //Множественный выбор
                    ...
                    $c->where(array('question_id'=>$q_id['id']));
                    $Answers = $modx->getIterator('UserTestAnswers', $c);
                    $selectedIds = usertestSafeIdListTraining(isset($q_id['ans']) ? $q_id['ans'] : array());
                    $Ans = array();
                    foreach($Answers as $a){
                        $a1 = $a->toArray();
                ...
                case 7: //Таблица чек-боксов
                    ...
                    $parentAns = usertestSafeArrayTraining(isset($q_id['ans']) ? $q_id['ans'] : array());
                    foreach($q_childs as $k_child=>$q_child){
                        ...
                        $childSelectedIds = usertestSafeIdListTraining(isset($parentAns[$q_child->id]) ? $parentAns[$q_child->id] : array());
                        $Ans = array();
                        foreach($Answers as $a){
                            $a1 = $a->toArray();
                            if (in_array((int)$a->id, $childSelectedIds, true)) {
                                $a1['check'] = 1;
                            }else{
                                $a1['check'] = 0;
                            }
                ...
                case 8: //Таблица текстовых полей
                    ...
                    foreach($q_childs as $k_child=>$q_child){
                        ...
                        foreach($Answers as $a){
                            $a1 = $a->toArray();
                            if(isset($parentAns[$q_child->id]) && is_array($parentAns[$q_child->id]) && isset($parentAns[$q_child->id][$a->id])) $a1['ac'] = $parentAns[$q_child->id][$a->id];
                            $Ans[] = $a1;
                        }
                        $question['header'] = $Ans_header;
                        $qc = $q_child->toArray();
                ...
                case 10: //Комбинированный одиночный выбор
                    ...
                    $Answers = $modx->getIterator('UserTestAnswers', $c);
                    $Ans = array();
                    $selectedCombo = usertestSafeArrayTraining(isset($q_id['ans']) ? $q_id['ans'] : array());
                    $selectedComboAnswer = isset($selectedCombo['ans']) ? $selectedCombo['ans'] : 0;
                    foreach($Answers as $a){
                        $a1 = $a->toArray();
                        if ($a->id == $selectedComboAnswer) {
                            $a1['check'] = 1;
                        }else{
                            $a1['check'] = 0;
                        }
                    ...
                    $question['answers'] = $Ans;
                    if($selectedComboAnswer=="ans_add"){
                        $question['answers_add'] = isset($q_id['ans_add']) ? $q_id['ans_add'] : array('check' => 0, 'ans' => '');
                    }else{
                        $question['answers_add'] = array(
                                    'check' => 0,
                                    'ans' => "",
            ...
            foreach($questions_post as $q_id=>$a_id){
                ...
                switch($q->type){
                    ...
                    case 12: //Опросник САН
                        ...
                        break;
                    case 2: //Множественный выбор
                        $a_id = usertestSafeIdListTraining($a_id);
                        foreach($steps[$resStep]['question_ids'] as $s_id=>$v){
                            if($v['id'] == $q_id){
                                $steps[$resStep]['question_ids'][$s_id]['ans'] = $a_id;
                            }
                    ...
                    case 5: //На сопоставление. Простой
                        ...
                        break;
                    case 6: //Комбинированный вариант
                        $a_id = usertestSafeArrayTraining($a_id);
                        foreach($steps[$resStep]['question_ids'] as $s_id=>$v){
                            if($v['id'] == $q_id){
                    ...
                    case 10: //Комбинированный одиночный выбор
                        ...
                        foreach($steps[$resStep]['question_ids'] as $s_id=>$v){
                            if($v['id'] == $q_id){
                                if(isset($a_id['ans']) && $a_id['ans']=="ans_add"){
                                    $steps[$resStep]['question_ids'][$s_id]['ans_add'] = array(
                                        'check' => 1,
                                        'ans' => $a_id['ans_add_ans'],
                                        );
                        ...
                        $resAns->question_id = $q_id;
                        $answer = ""; $point = 0;
                        $selectedComboAnswer = isset($a_id["ans"]) ? $a_id["ans"] : 0;
                        if($selectedComboAnswer == "ans_add"){
                            $resAns->answer_id = 0;
                            $answer = isset($a_id['ans_add_ans']) ? $a_id['ans_add_ans'] : '';
                            if($answer){
                            $answer = "Другое->".$answer; 
                            }
                        }else{
                            $resAns->answer_id = $selectedComboAnswer;
                            if($a = $modx->getObject('UserTestAnswers',$selectedComboAnswer)){
                                $answer = $a->answer;
                                $point = $a->point;
                            }
                        }
