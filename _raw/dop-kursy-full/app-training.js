...
(function () {
    ...
    function initPracticeMore(button) {
        ...
        var text = card ? card.querySelector('.practice-task-card__text') : null;

        function refreshMoreState() {
            if (!card || !text) {
                return;
            }
            ...
            var hasOverflow = text.scrollHeight > text.clientHeight + 4;

            if (wasOpen) {
                card.classList.add('is-open');
            }

            card.classList.toggle('has-no-more', !hasOverflow);
            ...
            if (!hasOverflow) {
                ...
                button.textContent = card.classList.contains('is-open') ? 'Свернуть' : 'Показать больше';
            }
        }
        ...
        window.setTimeout(refreshMoreState, 0);
        window.setTimeout(refreshMoreState, 250);

        if (window.ResizeObserver && text) {
            var ro = new ResizeObserver(refreshMoreState);
            ro.observe(text);
            ro.observe(card);
        } else {
            window.addEventListener('resize', refreshMoreState);
        }
    }
