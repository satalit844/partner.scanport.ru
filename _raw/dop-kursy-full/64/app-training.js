    ...
    return {
        ...
        usersCoursesUi: function () {
            ...
            function ensureMobileManageUi() {
                $wrap.find('.trigger-search').remove();
                ...
                if ($top.length && !$top.find('.training-manage-mobile-search-toggle').length) {
                    var $title = $top.find('.title-training').first();
                    var mobileButton = '' +
                        ...
                        '</button>';
                    if ($title.length) {
                        $(mobileButton).insertAfter($title);
                    } else {
                ...
                if ($row.length && !$row.find('.training-manage-search-cancel').length) {
                    $row.append('<button type="button" class="training-manage-search-cancel">Отменить</button>');
                }

                if ($searchBlock.length && !$searchBlock.find('.training-manage-search-results').length) {
                    $searchBlock.append('<div class="training-manage-search-results" aria-live="polite"></div>');
                }

                if ($searchBlock.length && !$wrap.find('.training-manage-search-chip').length) {
                    $('<div class="training-manage-search-chip" aria-live="polite"><span class="training-manage-search-chip__text"></span><button type="button" class="training-manage-search-chip__clear" aria-label="Очистить поиск">×</button></div>').insertAfter($searchBlock);
                }
            }
            ...
            function updateSearchChip() {
                ...
                var $chip = $wrap.find('.training-manage-search-chip').first();
                if (!$chip.length) {
                    return;
                }

                $chip.find('.training-manage-search-chip__text').text(query);
                $chip.toggleClass('is-visible', !!query && !$wrap.hasClass('is-mobile-search-open'));
            }
            ...
            function renderMobileSearchResults() {
                ...
                $.each(currentRows, function (_, item) {
                    ...
                    var haystack = (name + ' ' + email + ' ' + (item.organization || '')).toLowerCase();
                    ...
                    count++;
                    html += '' +
                        '<button type="button" class="training-manage-search-result" data-search-user-id="' + item.id + '" data-search-query="' + self.escapeHtml(name) + '">' +
                            ...
                            '<span class="training-manage-search-result__info">' +
                                ...
                                (email ? '<span class="training-manage-search-result__dot">•</span><span class="training-manage-search-result__email">' + self.escapeHtml(email) + '</span>' : '') +
                            '</span>' +
                        '</button>';
                });

                if (!count) {
                    html += '<div class="training-manage-search-results__empty">Ничего не найдено</div>';
                }
            ...
            function renderCards(rows) {
                ...
                $.each(rows, function (_, item) {
                    html += '' +
                        '<div class="course-card" data-user-card data-user-id="' + item.id + '" tabindex="0" role="checkbox" aria-checked="false">' +
                            ...
                            '<div class="course-meta">' +
                                ...
                                '</div>' +
                            '</div>' +
                            '<div class="course-card__select pt-2">' +
            ...
            $(document)
                ...
                .on('click.appTrainingMobileSearchClear', '#training-manage-page .training-manage-search-chip__clear, .training-manage-page .training-manage-search-chip__clear', function (e) {
                    e.preventDefault();
                    $search.val('');
                    closeMobileSearch();
                    updateSearchChip();
                ...
                .on('click.appTrainingMobileSearchResult', '#training-manage-page .training-manage-search-result, .training-manage-page .training-manage-search-result', function (e) {
                    e.preventDefault();
                    var query = $(this).attr('data-search-query') || $.trim($(this).find('.training-manage-search-result__name').text() || '');
                    $search.val(query);
                    closeMobileSearch();
                    updateSearchChip();
                    loadUsers();
            ...
            $(document)
                ...
                .on('click.appTrainingUserCardSelect', '#training-manage-page [data-user-card], .training-manage-page [data-user-card]', function (e) {
                    ...
                    var $card = $(this);
                    var userId = parseInt($card.attr('data-user-id'), 10) || 0;
                    if (!userId) return;
