...
$defaultTplFile = '@FILE ' . rtrim($modx->getOption('usertest_core_path', null, $modx->getOption('core_path') . 'components/usertest/'), '/\\') . '/elements/chunks/chunk.tpl.UserTest.main.tpl';
        ...
        if($q = $modx->getObject('UserTestQuestions', $q_id['id'])){
            ...
            switch($q->type){
                ...
                case 2: //Множественный выбор
                    ...
                    $Answers = $modx->getIterator('UserTestAnswers', $c);
                    $Ans = array();
                    $selectedAnswerIds = isset($q_id['ans']) ? usertestNormalizeIdsTraining($q_id['ans']) : array();
                    foreach($Answers as $a){
                        $a1 = $a->toArray();
                        if (in_array((int)$a->id, $selectedAnswerIds, true)) {
                            $a1['check'] = 1;
                        }else{
                            $a1['check'] = 0;
                        }
                    ...
                    $question['answers'] = $Ans;
                    $question['ans'] = $selectedAnswerIds;
            ...
            foreach($questions_post as $q_id=>$a_id){
                ...
                switch($q->type){
                    ...
                    case 12: //Опросник САН
                        ...
                        break;
                    case 2: //Множественный выбор
                        $selectedIds = usertestNormalizeIdsTraining($a_id);
                        foreach($steps[$resStep]['question_ids'] as $s_id=>$v){
                            if($v['id'] == $q_id){
                                $steps[$resStep]['question_ids'][$s_id]['ans'] = $selectedIds;
                            }
                        }
                        if(!$resAns = $modx->getObject('UserTestResultAnswers', array('result_id'=>$result_id, 'question_id'=>$q_id))){
                            $resAns = $modx->newObject('UserTestResultAnswers');
                        ...
                        $resAns->result_id = $result_id;
                        $resAns->question_id = $q_id;
                        $resAns->answer_ids = implode(',', $selectedIds);

                        $answer = array();
                        $point = 0;
                        $rightIds = array();
                        $rightAnswersQuery = $modx->newQuery('UserTestAnswers');
                        $rightAnswersQuery->select(array('id'));
                        $rightAnswersQuery->where(array(
                            'question_id' => $q_id,
                            'right' => 1,
                        ));
                        if($rightAnswers = $modx->getIterator('UserTestAnswers', $rightAnswersQuery)){
                            foreach($rightAnswers as $rightAnswer){
                                $rightIds[] = (int)$rightAnswer->get('id');
                            }
                        }
                        $rightIds = usertestNormalizeIdsTraining($rightIds);
                        $rightCount = count($rightIds);
                        $questionMaxPoint = (float)$q->get('max_point');
                        $pointPerRight = ($questionMaxPoint > 0 && $rightCount > 0) ? ($questionMaxPoint / $rightCount) : 0;

                        foreach($selectedIds as $a_id1){
                            if($a = $modx->getObject('UserTestAnswers',$a_id1)){
                                $answer[] = $a->answer;
                                if((int)$a->get('right') === 1){
                                    $answerPoint = (float)$a->get('point');
                                    $point += $answerPoint > 0 ? $answerPoint : $pointPerRight;
                                }
                            }
                        }

                        if($selectedIds === $rightIds && $questionMaxPoint > 0){
                            $point = $questionMaxPoint;
                        }

                        $resAns->answer = implode(', ', $answer);
                        $resAns->point = round($point, 2);
                        $resAns->save();
                        break;
