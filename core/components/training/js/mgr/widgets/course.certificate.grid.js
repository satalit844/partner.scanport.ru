...
Training.grid.CourseCertificates = function(config) {
    ...
    Ext.applyIf(config, {
        ...
        url: Training.config.connector_url,
        baseParams: {action: 'mgr/course/certificates/getlist', course_id: this.courseId},
        ...
        autoHeight: true,
        sm: this.sm,
        fields: ['id','user_id','display_name','email','completedon_formatted','issuedon_formatted','certificate_generated_label','file_path'],
        columns: [
            this.sm,
            ...
            {header:'Файл',dataIndex:'file_path',width:220,renderer: Training.utils.renderFileLink}
        ],
        viewConfig: {
            forceFit: false,
        ...
        tbar: [{
            ...
            minWidth: 180,
            handler: function() {
                var ids = Training.utils.getSelectedIds(this);
                if (!ids.length) {
...
Ext.extend(Training.grid.CourseCertificates, MODx.grid.Grid, {
    ...
    generate: function(ids) {
        ...
        MODx.Ajax.request({
            url: Training.config.connector_url,
            params: {action: 'mgr/course/certificate/generate', course_id: this.courseId, force: 1, user_ids: ids.join(',')},
            listeners: {
                success: {fn: function(r) {
                    this.getEl().unmask();
                    this.refresh();
