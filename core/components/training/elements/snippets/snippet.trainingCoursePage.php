...
if (!function_exists('trainingCoursePageFindActivityResourceId')) {
    function trainingCoursePageFindActivityResourceId(modX $modx, $linkType, $preferredResourceId = 0)
    {
        ...
        foreach ($ids as $id) {
            ...
            $content = (string)$resource->get('content');
            $uri = (string)$resource->get('uri');

            if ($linkType === 'test') {
                $isTrainingActivityPage = stripos($content, 'trainingActivityPage') !== false;
                $isOldCanonicalUri = stripos($uri, 'obuchenie-glavnaya/stranitsa-testa') !== false;
                $isDirectUserTestPage = stripos($content, 'UserTest') !== false && !$isTrainingActivityPage;

                if ($isTrainingActivityPage || $isOldCanonicalUri) {
                    return $id;
                }

                if ($isDirectUserTestPage) {
                    continue;
                }
            }
