var AppTraining = (function () {
    return {
        ...
        trainingPlayer: function () {
            ...
            $player.find('.js-player-start-over').text('Сначала');
            $player.find('.js-player-resume-continue').text('Продолжить');
            ...
            function setControlsVisible(state, autoHide) {
                state = state !== false;
                ...
                $player.toggleClass('is-controls-visible', state);

                if (state && autoHide !== false && !player.paused && !player.ended) {
            ...
            function normalizeResumeTime(seconds) {
                seconds = parseInt(seconds || 0, 10) || 0;

                if (currentData && currentData.current_video) {
                    var maxTime = parseInt(currentData.current_video.max_time || 0, 10) || 0;
                    var currentTime = parseInt(currentData.current_video.current_time || 0, 10) || 0;
                    if (seconds <= 0) {
                        seconds = Math.max(currentTime, maxTime);
                    }
                }

                var duration = Math.floor(player.duration || (currentData && currentData.current_video ? currentData.current_video.duration_seconds : 0) || 0);
            ...
            function updatePlayUi() {
                ...
                $player.toggleClass('is-playing-video', isPlaying);
                if (!isPlaying) {
            ...
            function isPseudoFullscreenActive() {
                return $player.hasClass('is-pseudo-fullscreen');
            }
            ...
            function isIosSafariLike() {
                ...
                var isWebKit = /WebKit/.test(ua) && !/CriOS|FxiOS|OPiOS|EdgiOS/.test(ua);
                return isiOS && isWebKit;
            }

            function lockPseudoFullscreen(state) {
                $('html, body').toggleClass('is-training-player-fullscreen-lock', !!state);
            }

            function setPseudoFullscreen(state) {
                state = !!state;
                $player.toggleClass('is-pseudo-fullscreen', state);
                lockPseudoFullscreen(state);
                setFullscreenState();
            }
            ...
            function applyNav(data) {
                ...
                if (nextItem) {
                    ...
                    $nextBtn
                        .removeClass('is-home')
                        .prop('disabled', nextLocked)
                        .attr('data-url', nextLocked ? '' : (nextItem.url || ''))
                        .attr('data-video', parseInt(nextItem.id || 0, 10))
                        .attr('data-locked', nextLocked ? '1' : '0')
                        .attr('data-home', '0');
                    setButtonLabel($nextBtn, 'Далее');
                } else {
                    var courseUrl = (currentData && currentData.course && currentData.course.url) ? currentData.course.url : ($backBtn.attr('data-url') || '');
                    $nextBtn
                        .addClass('is-home')
                        .prop('disabled', !courseUrl)
                        .attr('data-url', courseUrl || '')
                        .attr('data-video', '')
                        .attr('data-locked', '0')
                        .attr('data-home', courseUrl ? '1' : '0');
                    setButtonLabel($nextBtn, 'Главная');
                }
            }
            ...
            function showResumeModal(seconds, completedVideo) {
                pendingResumeTime = normalizeResumeTime(seconds);
            ...
            function setFullscreenState() {
                ...
                $fullscreenBtn.toggleClass('is-contract', active).attr('aria-label', active ? 'Выйти из полноэкранного режима' : 'На весь экран');
                if (!isPseudoFullscreenActive()) {
                    lockPseudoFullscreen(false);
                }
                movePlayerVideo();
                pokeControls();
            }
            ...
            function requestFullscreen() {
                ...
                if (isNativeFullscreenActive()) {
                    var exitPromise = exitNativeFullscreen();
            ...
            $(document)
                ...
                .on('click.appTrainingPlayerPrev', '.js-player-prev, .js-player-next', function () {
                    ...
                    var goHome = parseInt($btn.attr('data-home') || 0, 10) === 1 || $btn.hasClass('is-home');
                    if (goHome) {
                        if (url) {
                            window.location.href = url;
                        }
            ...
            $player.off('click.appTrainingResumeContinue').on('click.appTrainingResumeContinue', '.js-player-resume-continue', function () {
                hideResumeModal();
                var resumeTime = normalizeResumeTime(pendingResumeTime);
                try {
                    player.currentTime = resumeTime > 0 ? resumeTime : 0;
                    updateVideoProgress();
                } catch (e) {}
                var resumePlayPromise = player.play();
                if (resumePlayPromise && typeof resumePlayPromise.catch === 'function') {
                    resumePlayPromise.catch(function () {
                        updatePlayUi();
                    });
                }
            });
            ...
            $(document)
                ...
                .on('fullscreenchange.appTrainingPlayer webkitfullscreenchange.appTrainingPlayer mozfullscreenchange.appTrainingPlayer MSFullscreenChange.appTrainingPlayer', function () {
                    if (isNativeFullscreenActive() && isPseudoFullscreenActive()) {
                        $player.removeClass('is-pseudo-fullscreen');
                    }
                    setFullscreenState();
                })
                .off('keydown.appTrainingPlayerFullscreen')
                .on('keydown.appTrainingPlayerFullscreen', function (e) {
            ...
            $(player)
                ...
                .on('loadedmetadata.appTrainingPlayer', function () {
                    ...
                    if (shouldPromptResume) {
                        shouldPromptResume = false;
                        var resume = normalizeResumeTime(currentData.current_video.resume_time || currentData.current_video.current_time || currentData.current_video.max_time || 0);
            ...
            $player
                .off('mousemove.appTrainingControls pointermove.appTrainingControls pointerdown.appTrainingControls touchstart.appTrainingControls focusin.appTrainingControls')
                .on('mousemove.appTrainingControls pointermove.appTrainingControls pointerdown.appTrainingControls touchstart.appTrainingControls focusin.appTrainingControls', function () {
                    pokeControls();
                });

            $(window)
                .off('resize.appTrainingPlayerMoveVideo')
                .on('resize.appTrainingPlayerMoveVideo', function () {
                    movePlayerVideo();
                });
        ...
        filtersLogic: function () {
            var $blocks = $('.my-courses, .certificates-page, [data-certificates-page]');
            ...
            function setActive($scope, $btn) {
                $scope.find('.filters-corses .course-filter__chip, .certificates-page__filters button, .certificates-page__filters a, .certificates-filters button, .certificates-filters a, .certificates-page__filter')
            ...
            function getItemsContainer($scope) {
                var $list = $scope.find('.corses-items').first();
                if ($list.length) return $list;

                $list = $scope.find('.sert-items, .certificates-page__items').first();
                if ($list.length) return $list;

                $list = $scope.find('*').filter(function () {
                    return $(this).find('.course-item, .sert-item, .cert-item').length;
                }).first();
            ...
            function normalizeFilter(filter) {
                ...
                if (filter === 'acting' || filter === 'current' || filter === 'valid' || filter === 'deystvuet') return 'active';
                if (filter === 'expired' || filter === 'expire' || filter === 'old' || filter === 'istek') return 'expired';
                return filter;
            }

            function getButtonFilter($btn) {
                var filter = $btn.attr('data-cert-filter')
                    || $btn.data('certFilter')
                    || $btn.attr('data-filter')
                    || $btn.data('filter')
                    || $btn.attr('data-status')
                    || '';

                if (!filter) {
                    var text = $.trim($btn.text() || '').toLowerCase();
                    if (text.indexOf('действ') !== -1) filter = 'active';
                    else if (text.indexOf('ист') !== -1) filter = 'expired';
                    else filter = 'all';
                }
            ...
            function itemMatches($item, filter) {
                ...
                var cls = map[filter] || ('is-' + filter);
                var text = $.trim($item.text() || '').toLowerCase();
                var status = String(
                    $item.attr('data-cert-status')
                    || $item.attr('data-cert-state')
                    || $item.attr('data-certificate-status')
                    || $item.attr('data-status')
                    || $item.attr('data-filter')
                    || ''
                ).toLowerCase();

                var expiredFlag = String($item.attr('data-expired') || '').toLowerCase();

                if ($item.hasClass(cls) || $item.hasClass('is-' + filter) || status === filter) {
                    return true;
                }
                ...
                if (filter === 'active') {
                    return status === 'valid'
                        ...
                        || expiredFlag === '0'
                        || expiredFlag === 'false'
                        || $item.hasClass('is-valid')
                        || text.indexOf('действ') !== -1
                        || (!$item.hasClass('is-done') && !$item.hasClass('is-expired') && text.indexOf('ист') === -1);
                }
                ...
                if (filter === 'expired') {
                    return status === 'expired'
                        ...
                        || expiredFlag === '1'
                        || expiredFlag === 'true'
                        || $item.hasClass('is-expired')
                        || $item.hasClass('is-done')
                        || text.indexOf('ист') !== -1;
                }
            ...
            function getEmptyText($scope) {
                if ($scope.is('.certificates-page, [data-certificates-page]') || $scope.find('.sert-item, .cert-item').length) {
                    return 'Сертификатов пока нет';
                }
                return 'Ничего не найдено';
            }

            function applyFilter($scope, filter) {
                var $list = getItemsContainer($scope);
                var $items = $list.find('.course-item, .sert-item, .cert-item');
                if (!$items.length) {
                    return;
                }
                ...
                $items.each(function () {
                    var $item = $(this);
                    var show = itemMatches($item, filter);
                    $item.toggle(show);
                    if (show) visible++;
                });
                ...
                if (!$empty.length) {
                    $empty = $('<div class="training-filter-empty"></div>').appendTo($list);
                }
                $empty.text(getEmptyText($scope)).toggle(visible === 0);
            }

            $(document)
                .off('click.appTrainingFilters')
                .on('click.appTrainingFilters', '.my-courses .filters-corses .course-filter__chip, .certificates-page .filters-corses .course-filter__chip, [data-certificates-page] .filters-corses .course-filter__chip, .certificates-page__filters .cert-filter__chip, .certificates-page__filters button, .certificates-page__filters a, .certificates-filters button, .certificates-filters a, .certificates-page__filter', function (e) {
                    e.preventDefault();

                    var $btn = $(this);
                    var $scope = $btn.closest('.my-courses, .certificates-page, [data-certificates-page]');
                    if (!$scope.length) return;

                    var filter = getButtonFilter($btn);
                    setActive($scope, $btn);
            ...
            $blocks.each(function () {
                var $scope = $(this);
                var $active = $scope.find('.filters-corses .course-filter__chip.is-active, .certificates-page__filters .cert-filter__chip.is-active, .certificates-page__filters .is-active, .certificates-page__filters .active, .certificates-filters .is-active, .certificates-filters .active, .certificates-page__filter.is-active, .certificates-page__filter.active').first();
                var $first = $scope.find('.filters-corses .course-filter__chip, .certificates-page__filters .cert-filter__chip, .certificates-page__filters button, .certificates-page__filters a, .certificates-filters button, .certificates-filters a, .certificates-page__filter').first();
                var $initBtn = $active.length ? $active : $first;

                if ($initBtn.length) {
                    setActive($scope, $initBtn);
                    applyFilter($scope, getButtonFilter($initBtn));
                }
            });
        },
        ...
        usersCoursesUi: function () {
            ...
            function isManageMobile() {
                return window.matchMedia && window.matchMedia('(max-width: 991.98px)').matches;
            }

            function ensureMobileManageUi() {
                var $top = $wrap.children('.d-flex.align-items-center.justify-content-between, .d-flex.align-items-start.justify-content-between').first();
                if ($top.length && !$top.find('.training-manage-mobile-search-toggle').length) {
                    $('<button type="button" class="training-manage-mobile-search-toggle" aria-label="Открыть поиск"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M21 21L16.65 16.65M19 11C19 15.4183 15.4183 19 11 19C6.58172 19 3 15.4183 3 11C3 6.58172 6.58172 3 11 3C15.4183 3 19 6.58172 19 11Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></button>').insertAfter($top.find('.title-training').first());
                }

                var $searchBlock = $wrap.find('.search-block').first();
                if ($searchBlock.length) {
                    var $row = $searchBlock.children('.d-flex').first();
                    if (!$row.length) {
                        $row = $searchBlock;
                    }

                    if (!$row.find('.training-manage-search-cancel').length) {
                        $row.append('<button type="button" class="training-manage-search-cancel">Отменить</button>');
                    }

                    if (!$wrap.find('.training-manage-search-results').length) {
                        $('<div class="training-manage-search-results" aria-live="polite"></div>').insertAfter($searchBlock);
                    }
                }
            }

            function renderMobileSearchResults() {
                var $results = $wrap.find('.training-manage-search-results').first();
                if (!$results.length) return;

                var query = $.trim($search.val() || '').toLowerCase();
                var rows = currentRows || [];
                var shown = 0;
                var html = '<div class="training-manage-search-results__title">' + (query ? 'Результаты поиска:' : 'Недавние запросы:') + '</div>';

                $.each(rows, function (_, item) {
                    if (shown >= 8) return false;
                    var name = item.display_name || item.username || '';
                    var email = item.email || '';
                    var haystack = (name + ' ' + email).toLowerCase();
                    if (query && haystack.indexOf(query) === -1) {
                        return;
                    }
                    shown++;
                    html += '' +
                        '<button type="button" class="training-manage-search-result" data-search-user-id="' + self.escapeHtml(item.id || '') + '" data-search-query="' + self.escapeHtml(name) + '">' +
                            '<span class="training-manage-search-result__avatar" aria-hidden="true"><img src="theme/images/training/user-ico.svg" class="img-svg" alt=""></span>' +
                            '<span class="training-manage-search-result__info">' +
                                '<span class="training-manage-search-result__name">' + self.escapeHtml(name) + '</span>' +
                                '<span class="training-manage-search-result__dot"> · </span>' +
                                '<span class="training-manage-search-result__email">' + self.escapeHtml(email) + '</span>' +
                            '</span>' +
                        '</button>';
                });

                if (!shown) {
                    html += '<div class="training-manage-search-result training-manage-search-result--empty">Пользователи не найдены</div>';
                }
            ...
            function openMobileSearch() {
                ensureMobileManageUi();
                if (!isManageMobile()) return;
                $wrap.addClass('is-mobile-search-open');
            ...
            function closeMobileSearch() {
                $wrap.removeClass('is-mobile-search-open');
            }
            ...
            function syncSelectionState(changedUserId, changedChecked) {
                ...
                $wrap.find('[data-user-row], [data-user-card]').each(function () {
                    var $item = $(this);
                    var id = parseInt($item.attr('data-user-id'), 10) || 0;
                    var selected = !!selectedMap[id];
                    $item.toggleClass('is-selected', selected).attr('aria-checked', selected ? 'true' : 'false');
                });

                $wrap.toggleClass('has-selected-users', selectedIds.length > 0);
                $wrap.find('.action-panel').toggleClass('is-mobile-visible', selectedIds.length > 0);
            ...
            $(document)
                .off('click.appTrainingMobileSearchOpen')
                .on('click.appTrainingMobileSearchOpen', '#training-manage-page .training-manage-mobile-search-toggle, .training-manage-page .training-manage-mobile-search-toggle', function (e) {
                    e.preventDefault();
                    e.stopPropagation();
                    openMobileSearch();
                })
                .off('click.appTrainingMobileSearchCancel')
                .on('click.appTrainingMobileSearchCancel', '#training-manage-page .training-manage-search-cancel, .training-manage-page .training-manage-search-cancel', function (e) {
                    e.preventDefault();
                    closeMobileSearch();
                })
                .off('click.appTrainingMobileSearchResult')
                .on('click.appTrainingMobileSearchResult', '#training-manage-page .training-manage-search-result, .training-manage-page .training-manage-search-result', function (e) {
                    e.preventDefault();
                    var $btn = $(this);
                    if ($btn.hasClass('training-manage-search-result--empty')) return;
                    var query = $btn.attr('data-search-query') || $.trim($btn.find('.training-manage-search-result__name').text() || '');
                    $search.val(query);
                    closeMobileSearch();
                    loadUsers();
                });
            ...
            $(document)
                .off('click.appTrainingUserCardSelect')
                .on('click.appTrainingUserCardSelect', '#training-manage-page [data-user-card], .training-manage-page [data-user-card]', function (e) {
                    if ($(e.target).closest('a, button, input, label').length) return;
                    var userId = parseInt($(this).attr('data-user-id'), 10) || 0;
                    if (!userId) return;
                    var checked = !$wrap.find('.users-check[data-user-id="' + userId + '"]').first().prop('checked');
                    syncSelectionState(userId, checked);
                })
                .off('keydown.appTrainingUserCardSelect')
                .on('keydown.appTrainingUserCardSelect', '#training-manage-page [data-user-card], .training-manage-page [data-user-card]', function (e) {
...
/* ===== training manage: isolated mobile search modal override ===== */
(function () {
    function initTrainingManageSearchModal() {
        var $wrap = $('#training-manage-page[data-training-page="managecourses"], .training-manage-page[data-training-page="managecourses"]').first();
        if (!$wrap.length) return;

        var $mainInput = $wrap.find('#users-search .search-input__field').first();
        if (!$mainInput.length) return;

        var $modal = $('#trainingManageSearchModal');
        ...
        function ensureModal() {
            $modal = $('#trainingManageSearchModal');
            if ($modal.length) return $modal;

            var html = '' +
                '<div class="training-manage-search-modal" id="trainingManageSearchModal" aria-hidden="true">' +
                    '<div class="training-manage-search-modal__inner">' +
                        '<div class="training-manage-search-modal__top">' +
                            '<label class="training-manage-search-modal__field">' +
                                '<span class="training-manage-search-modal__ico" aria-hidden="true">' +
                                ...
                                '</span>' +
                                '<input type="text" class="training-manage-search-modal__input" placeholder="Поиск по сотрудникам" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" inputmode="search">' +
                                '<button type="button" class="training-manage-search-modal__clear" aria-label="Очистить поиск">×</button>' +
                            '</label>' +
                            '<button type="button" class="training-manage-search-modal__cancel">Отменить</button>' +
                        '</div>' +
                        '<div class="training-manage-search-modal__results" aria-live="polite"></div>' +
                    '</div>' +
                '</div>';
        ...
        function sourceResultsHtml() {
            var $source = $wrap.find('.training-manage-search-results').first();
            var html = $source.length ? $.trim($source.html() || '') : '';
            return html;
        }

        function fallbackResultsHtml(query) {
            query = $.trim(query || '').toLowerCase();
            var rows = [];
            var seen = {};

            $wrap.find('[data-user-card]').each(function () {
                var $card = $(this);
                var id = $card.attr('data-user-id') || '';
                if (!id || seen[id]) return;
                seen[id] = true;
                rows.push({
                    id: id,
                    name: $.trim($card.find('.course-name').first().text() || ''),
                    email: $.trim($card.find('.course-email').first().text() || '')
                });
            });

            $wrap.find('[data-user-row]').each(function () {
                var $row = $(this);
                ...
                rows.push({
                    id: id,
                    name: $.trim($row.find('.user-mini__name').first().text() || ''),
                    email: $.trim($row.find('.user-mini__mail').first().text() || '')
                });
            });

            var title = query ? 'Результаты поиска:' : 'Недавние запросы:';
            var html = '<div class="training-manage-search-results__title">' + title + '</div>';
            var shown = 0;
            ...
            $.each(rows, function (_, item) {
                ...
                shown++;
                html += '' +
                    '<button type="button" class="training-manage-search-result" data-search-user-id="' + escapeHtml(item.id) + '" data-search-query="' + escapeHtml(name) + '">' +
                        '<span class="training-manage-search-result__avatar" aria-hidden="true"><img src="theme/images/training/user-ico.svg" class="img-svg" alt=""></span>' +
                        '<span class="training-manage-search-result__info">' +
                            '<span class="training-manage-search-result__name">' + escapeHtml(name) + '</span>' +
                            '<span class="training-manage-search-result__dot"> · </span>' +
                            '<span class="training-manage-search-result__email">' + escapeHtml(email) + '</span>' +
                        '</span>' +
                    '</button>';
            });
        ...
        function syncModalResults() {
            var $m = ensureModal();
            var query = $m.find('.training-manage-search-modal__input').val() || '';
            var html = sourceResultsHtml();
            if (!html) {
                html = fallbackResultsHtml(query);
            }
            $m.find('.training-manage-search-modal__results').html(html);
            $m.toggleClass('has-value', $.trim(query) !== '');
        }

        function refreshResultsSoon() {
            syncModalResults();
            window.setTimeout(syncModalResults, 80);
            window.setTimeout(syncModalResults, 320);
        }

        function openModal() {
            if (!isMobile()) return;
            var $m = ensureModal();
            $wrap.removeClass('is-mobile-search-open is-search-open search-open');
            $m.find('.training-manage-search-modal__input').val($mainInput.val() || '');
            $m.addClass('is-open').attr('aria-hidden', 'false');
            $('body').addClass('training-manage-search-modal-open');
            refreshResultsSoon();
            window.setTimeout(function () {
                $m.find('.training-manage-search-modal__input').trigger('focus');
            }, 30);
        }

        function closeModal() {
            $wrap.removeClass('is-mobile-search-open is-search-open search-open');
            ensureModal().removeClass('is-open has-value').attr('aria-hidden', 'true');
            $('body').removeClass('training-manage-search-modal-open');
        }
        ...
        $(document)
            ...
            .off('click.appTrainingMobileSearchCancel')
            .on('click.appTrainingMobileSearchCancel', '#trainingManageSearchModal .training-manage-search-modal__cancel', function (e) {
                e.preventDefault();
                closeModal();
            })
            .off('click.appTrainingMobileSearchClear')
            .on('click.appTrainingMobileSearchClear', '#trainingManageSearchModal .training-manage-search-modal__clear', function (e) {
                e.preventDefault();
                var $input = ensureModal().find('.training-manage-search-modal__input');
                $input.val('');
                $mainInput.val('').trigger('input');
                refreshResultsSoon();
                $input.trigger('focus');
            })
            .off('input.appTrainingMobileSearchModal')
            .on('input.appTrainingMobileSearchModal', '#trainingManageSearchModal .training-manage-search-modal__input', function () {
                $mainInput.val($(this).val() || '').trigger('input');
                refreshResultsSoon();
            })
            .off('click.appTrainingMobileSearchResult')
            .on('click.appTrainingMobileSearchResult', '#trainingManageSearchModal .training-manage-search-result', function (e) {
                e.preventDefault();
                var $btn = $(this);
                if ($btn.hasClass('training-manage-search-result--empty')) return;
                var query = $btn.attr('data-search-query') || $.trim($btn.find('.training-manage-search-result__name').text() || '');
                $mainInput.val(query).trigger('input');
                ensureModal().find('.training-manage-search-modal__input').val(query);
                closeModal();
            })
            .off('keydown.appTrainingMobileSearchModal')
            .on('keydown.appTrainingMobileSearchModal', function (e) {
                if (e.key === 'Escape' && $('#trainingManageSearchModal').hasClass('is-open')) {
                    closeModal();
                }
            });

        $(window)
            .off('resize.appTrainingMobileSearchModal')
            .on('resize.appTrainingMobileSearchModal', function () {
                if (!isMobile()) {
                    closeModal();
                }
            });
        ...
        $(document)
            .off('ajaxComplete.appTrainingMobileSearchModal')
            .on('ajaxComplete.appTrainingMobileSearchModal', function () {
                if ($('#trainingManageSearchModal').hasClass('is-open')) {
                    syncModalResults();
                }
            });
    }
    ...
    if (window.jQuery) {
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', initTrainingManageSearchModal);
        } else {
            initTrainingManageSearchModal();
        }
    }
})();
